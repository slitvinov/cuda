/*
   Scaling sweep for one Gauss-Newton iteration.

   One iteration = forward + Jacobian + residual + Cholesky solve +
   apply step.  All four kernels run per iteration; the timed loop
   measures the cost of one full GN iteration end-to-end.
*/
...
static double bench(int G, int gn, int N, int reps) {
    ...
    float   *d_params, *d_params_trial, *d_inputs, *d_targets,
            *d_state_v, *d_state_t, *d_out_v, *d_J, *d_r;
    uint8_t *d_genome;
    double  *d_delta;
...
#undef MALLOC
...

    {
        ...
        cholesky_solve_kernel<<<G, 128, smem_bytes>>>(d_J, d_r, d_delta, 1e-3, m, n_p);
        ...
        cudaFree(d_state_t); cudaFree(d_out_v); cudaFree(d_J);
        cudaFree(d_r); cudaFree(d_delta);
        return (double)ms / reps;
    }

oom:
    /* Best-effort free of whatever was allocated. */
    return -1;
launch_err:
    cudaFree(d_params); cudaFree(d_params_trial); cudaFree(d_inputs);
    cudaFree(d_targets); cudaFree(d_genome); cudaFree(d_state_v);
...
int main(void) {
    ...
    printf("One Gauss-Newton iteration  (forward+J + residual + Cholesky + apply).\n\n");
