/*
   ...
   doubling the state buffer and the per-op work.

   Build:  nvcc -arch=sm_80 -O2 bench_dgp.cu -o bench_dgp
   Run:    ./bench_dgp
*/
...
__global__ void forward_kernel_ad(const float   *inputs,
...
static double bench(int G, int gn, int N, int reps) {
    ...
    if (cudaMalloc(&d_state_v, state_bytes)  != cudaSuccess) { cudaFree(d_inputs); cudaFree(d_genome); return -1; }
    if (cudaMalloc(&d_state_d, state_bytes)  != cudaSuccess) { cudaFree(d_inputs); cudaFree(d_genome); cudaFree(d_state_v); return -1; }
    if (cudaMalloc(&d_out_v,   out_bytes)    != cudaSuccess) { cudaFree(d_inputs); cudaFree(d_genome); cudaFree(d_state_v); cudaFree(d_state_d); return -1; }
    if (cudaMalloc(&d_out_d,   out_bytes)    != cudaSuccess) { cudaFree(d_inputs); cudaFree(d_genome); cudaFree(d_state_v); cudaFree(d_state_d); cudaFree(d_out_v); return -1; }
    ...
    forward_kernel_ad<<<G, B>>>(d_inputs, d_genome, d_state_v, d_state_d, d_out_v, d_out_d, gn, N);
...
int main(void) {
    ...
    printf("Differentiable forward (value + tangent).\n\n");
    ...
    for (int ni     = 0; ni     < (int)(sizeof(Ns)/sizeof(Ns[0]));   ++ni) {
        ...
        /* HBM traffic now: inputs + genome + 2*state R/W + 2*out write. */
